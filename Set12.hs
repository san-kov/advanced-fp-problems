...
instance Functor List where
  fmap f Empty = Empty
  fmap f (LNode node sub) = LNode (f node) (fmap f sub)
...
instance Functor TwoList where
  ...
  fmap f (TwoNode l r sub) = TwoNode (f l) (f r) (fmap f sub)
...
count val = foldr (\x res -> if val == x then 1 + res else res) 0
...
inBoth f1 f2 = foldr (\x res -> if elem x f2 then x : res else res) [] f1
...
instance Foldable List where
  ...
  foldr fn acc (LNode node sub) = fn node (foldr fn acc sub)
...
instance Foldable TwoList where
  ...
  foldr fn acc (TwoNode l r sub) = fn l (fn r (foldr fn acc sub))
