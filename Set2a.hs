...
updateAt :: Int -> a -> [a] -> [a]
updateAt i x xs = firstPart ++ (x:secondPart)
                where firstPart = (take i xs)
                      secondPart = (drop (i + 1) xs)
...
-- Examples:
--   isPalindrome ""         ==>  True
--   isPalindrome "ABxBA"     ==>  True
--   isPalindrome "racecar"  ==>  True
--   isPalindrome "AB"       ==>  False

isPalindrome :: String -> Bool
isPalindrome str = isPalindrome' str

isPalindrome' :: String -> Bool
isPalindrome' [] = True 
isPalindrome' [_] = True
isPalindrome' currentWord = if firstLetter == lastLetter 
                            then isPalindrome' restWord
                            else False 
                        where firstLetter = head currentWord
                              lastLetter = last currentWord
                              len = length currentWord
                              restWord = substring 1 (len - 1) currentWord
...
palindromify' str = if (isPalindrome str)
                    then str 
                    else palindromify' (substring 1 (len - 1) str)
