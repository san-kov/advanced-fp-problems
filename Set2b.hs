...
oddFactorial' n result = if even n 
                         then oddFactorial' (n - 1) result 
                         else oddFactorial' (n - 1) result * n
------------------------------------------------------------------------------
...
myGcd 0 b = b  
myGcd a b = if (a > b) 
            then myGcd newNumber b 
            else myGcd a newNumber
        where newNumber = abs (a - b)
...
leftpad str n 
        | n <= strLength = str 
        | otherwise = leftpad' str (n - strLength)
...
smallestDivisor' n current = if (mod n current) == 0
                            then current 
                            else smallestDivisor' n (current + 1)
