...
allEqual [] = True
allEqual (x:xs) = foldr (\cur prev -> cur == x && prev) True xs
...
longest xs = foldr comp [] xs
        where comp cur prev = if curlen > prevlen
                              then cur 
                              else if curlen == prevlen
                                then if (head cur) < (head prev) then cur else prev 
                                else prev
                where curlen = (length cur)
                      prevlen = (length prev)
------------------------------------------------------------------------------
...
average xs = (foldr (\x sum -> x + sum) 0 xs) / (fromIntegral (length xs) :: Fractional a => a) 
...
freqs xs = foldr (\val res -> Map.alter incCount val res) (Map.fromList []) xs
        where incCount (Just val) = Just (val + 1)
...
transfer from to amount bank = if 
                                Map.notMember from bank 
                                || Map.notMember to bank 
                                || amount < 0 
                                || emptyBalance
                               then bank 
                               else firstUpdate . secondUpdate $ bank
                where firstUpdate = Map.alter (alterBalance (-amount)) $ from 
                      secondUpdate = Map.alter (alterBalance amount) to
                      emptyBalance = case Map.lookup from bank of
                                        Nothing -> False 
                                        (Just balance) -> (balance - amount < 0)      
                      alterBalance _ (Nothing) = Nothing  
                      alterBalance val (Just balance) = (Just (balance + val)) 
...
maxIndex arr = findMax (indices arr) (arr ! firstIdx) firstIdx
        where   
              findMax [] max maxIx = maxIx  
              findMax (i:ix) max maxIx = let el = (arr ! i) in  
                                    if el >= max then findMax ix el i else findMax ix max maxIx
              firstIdx = (head .indices $ arr)
