...
countHelper x prev = maybe (prev + 1) (\_ -> prev) x 
------------------------------------------------------------------------------
...
concatStart = []
concatHelper list lists = list ++ lists 
...
largestHelper cur (x:xs) = if cur > x then [cur] else if cur == x then cur:x:xs else x:xs
...
myLast :: [a] -> Maybe a
myLast xs = foldl lastHelper Nothing xs

lastHelper Nothing c = Just c 
lastHelper c prev = c
