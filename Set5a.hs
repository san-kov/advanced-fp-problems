...
buyOneMore (MkShoppingEntry n price count) = (MkShoppingEntry n price (count + 1))
...
setName name (MkPerson a n) = (MkPerson a name)
...
data UpDown = IncCount Int | DecCount Int
...
tick :: UpDown -> UpDown
tick (IncCount n) = (IncCount (n + 1))
tick (DecCount n) = (DecCount (n - 1))

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (IncCount x) = (DecCount x)
toggle (DecCount x) = (IncCount x)
...
toNat z 
  | z < 0 = Nothing
  | otherwise = natResult
    where natResult = case toNat(z - 1) of
                        Nothing -> Nothing
...
fromBin :: Bin -> Int
fromBin bin = fromBin' bin 0 0

fromBin' (O End) pow res = res 
fromBin' (I End) pow res = res + 2^pow
fromBin' (O bin) pow res = fromBin' bin (pow + 1) res
fromBin' (I bin) pow res = fromBin' bin (pow + 1) (res + 2^pow) 
