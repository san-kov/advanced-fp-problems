...
treeMax Empty = 0
treeMax (Node v a b) = if v > largestA then (if v > largestB then v else largestB) 
                      else (if largestA > largestB then largestA else largestB)
  where largestA = treeMax a 
        largestB = treeMax b 
...
cull val (Node v a b) = if v == val then Empty else (Node v (cull val a) (cull val b)) 
...
isOrdered (Node v (Node v1 a b) Empty) = v > v1 && isOrdered (Node v1 a b) 
isOrdered (Node v Empty (Node v1 a b)) = v < v1  && isOrdered (Node v1 a b)
isOrdered (Node v (Node v1 a1 b1) (Node v2 a2 b2)) = (v > v1) && (v < v2) && isOrdered (Node v1 a1 b1) && isOrdered (Node v2 a2 b2)  
...
walk (StepL:xs) (Node v a b) = walk xs a 
walk (StepR:xs) (Node v a b) = walk xs b
...
set xs a Empty = Empty
set [StepL] val (Node v Empty t) = (Node v Empty t)
set [StepR] val (Node v t Empty) = (Node v t Empty)
set [] val (Node v a b) = (Node val a b)
set (StepL:xs) val (Node v a b) = (Node v (set xs val a) b)
set (StepR:xs) val (Node v a b) = (Node v a (set xs val b))
...
search val (Node v a b) = if val == v 
                          then Just [] 
                          else case (searchResA, searchResB) of 
                              (Nothing, Nothing) -> Nothing 
                              (Nothing, (Just xs)) -> Just (StepR:xs)   
                              ((Just xs), Nothing) -> Just (StepL:xs)   
          where searchResA = search val a 
                searchResB = search val b 
