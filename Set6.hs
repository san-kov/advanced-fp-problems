...
instance Ord Country where
  compare Norway Finland = GT   -- implement me?
...
instance Num RationalNumber where
  (RationalNumber n1 d1) + (RationalNumber n2 d2) = simplify $ RationalNumber ((d2 * n1) + (n2 * d1)) (d1 * d2)
  ...
  signum (RationalNumber n d) | n == 0 = RationalNumber 0 1
...
class Cycle a where 
  ...
  stepMany :: Int -> a -> a
  stepMany 0 c = c
  stepMany n c = step (stepMany (n - 1) c)
