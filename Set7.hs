...
member el (Set []) = False
member el (Set (x:xs)) = if el == x then True else (member el (Set xs))

-- add a member to a set
add :: Ord a => a -> Set a -> Set a
add el (Set []) = Set [el]
add el (Set (x:xs)) = if el == x then Set (x:xs) else if el < x then Set (el:x:xs) else Set (x:added)
    where (Set added) = add el (Set xs)
...
data State = Start | EggsAdded | FlourFirstAdded | SugarFirstAdded | ReadyToMix | Mixed | Error | Finished
  deriving (Eq,Show)

step Start AddEggs = EggsAdded

step EggsAdded AddFlour = FlourFirstAdded
step FlourFirstAdded AddSugar = ReadyToMix

step EggsAdded AddSugar = SugarFirstAdded
step SugarFirstAdded AddFlour = ReadyToMix
...
reverseNonEmpty (x :| []) = (x :| [])
reverseNonEmpty (x :| xs) = x1 :| xs1 ++ [x]
      where (x1:xs1) = (reverse xs)
...
passwordAllowed pass (And r1 r2) = passwordAllowed pass r1 && passwordAllowed pass r2
passwordAllowed pass (Or r1 r2) = passwordAllowed pass r1 || passwordAllowed pass r2
...
data Arithmetic = Literal Integer | Addit Arithmetic Arithmetic | Mplcat Arithmetic Arithmetic 
  deriving Show
...
operation "+" a b = (Addit a b)
operation "*" a b = (Mplcat a b)

evaluate :: Arithmetic -> Integer
evaluate (Literal x) = x 
evaluate (Addit x y) = (evaluate x) + (evaluate y) 
evaluate (Mplcat x y) = (evaluate x) * (evaluate y) 

render :: Arithmetic -> String
render (Literal x) = show x
render (Addit x y) = "(" ++ (render x) ++ "+" ++ (render y) ++ ")"
render (Mplcat x y) = "(" ++ (render x) ++ "*" ++ (render y) ++ ")"